...
This is the end-to-end process for implementing any non-trivial feature. Follow it in order — do not skip steps.
...
**Example:** `/new-proposal user-profile`
...
**Example:** `/new-spec 0001-user-profile`

Claude will ask you:

- Any existing data models or collections to reuse?
...
The `architect` agent then writes `tech-specs/NNNN-slug.md` with the full Clean Architecture file map, Dart interfaces, data schema, and test plan.
...
**Example:** `/new-feature user-profile`
...
Hand the approved spec and the session scratchpad to the `flutter-engineer` agent. Before writing any code, the engineer will ask you about open UI decisions:
...
- [ ] Domain entities defined — zero Flutter/backend imports
- [ ] Repository interfaces in `domain/repositories/`
- [ ] Data source in `data/datasources/`
...
**Example:** `/pr-review feat/user-profile`
...
| `security-reviewer` | Auth flows, API surface, secret handling            |
...
**Example:** `/debug Profile screen crashes when navigating back`
...
| `security-reviewer` | Audit auth, API surface, secrets          | Approve own audits |
| `release-engineer`  | Cut tags, release notes, store submission | Write feature code |
