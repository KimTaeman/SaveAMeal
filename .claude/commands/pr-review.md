...
Example: `/pr-review 42` or `/pr-review feat/user-profile`
...
2. **Fan out in parallel** — invoke all three reviewers with the diff as context:
   ...
   - `@security-reviewer` — threat model + secret scan + API surface audit
...
- Architecture violations (domain layer importing Flutter/backend) are always blocking
