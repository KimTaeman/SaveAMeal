...
1. **Confirm the feature name** — convert to snake_case if needed (e.g. `user-profile` → `user_profile`)
...
<name>/
  data/
    datasources/   ← empty, add backend calls here
...
3. **Create a session scratchpad** at `docs/sessions/YYYY-MM-DD-<name>.md` from the template in
   ...
   - Plan: list the domain entities needed, the data sources involved, and the screens required
