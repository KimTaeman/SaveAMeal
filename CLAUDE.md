...
**[Project name TBD]** is a cross-platform mobile app (iOS, Android, Web) built with Flutter.

> Backend and database tech stack TBD. The Domain layer is backend-agnostic by design — datasource implementations will be swapped in once the stack is decided.
...
│   └── commands/        ← /debug, /new-feature, /new-proposal, /new-spec, /pr-review
├── apps/
...
Strict Clean Architecture — the Domain layer must have **zero Flutter or backend imports**.

```
apps/mobile/lib/
  features/<name>/
    data/
      datasources/     ← backend calls, DTOs (implementation TBD)
...
- **architect** — system design, schema design, and PR review only. Cannot write feature code.
...
- **security-reviewer** — audits auth flows, API surface, and secrets. Read-only reviewer.
- **release-engineer** — owns the release pipeline, tag cutting, and store submission.
...
The **architect** writes a proposal using the stencil at `docs/stencils/tech-proposal.md`. Covers: problem, proposed solution, alternatives considered, and open questions. The team approves before moving on.

> Skip for changes touching ≤ 2 files with no architectural impact.

### 2. Tech Spec (`tech-specs/NNNN-slug.md`)
The **architect** expands the approved proposal into a full spec using `docs/stencils/tech-spec.md`. Covers: Clean Architecture layer breakdown, data schema, Riverpod providers, acceptance criteria, and test plan.
...
- Always use package imports (`package:mobile/...`) — never relative imports (`../..`). Enforced by `always_use_package_imports` lint; run `dart fix --apply` to fix violations.
...
**Proposals** (`tech-proposals/`) — use the stencil at `docs/stencils/tech-proposal.md`. Must be approved before a spec is written.

**Specs** (`tech-specs/`) — use the stencil at `docs/stencils/tech-spec.md`. Must reference the approved proposal. Implementation only begins once the spec is approved.
...
**Agent runs** (`docs/agent-runs/`) — every reviewer agent writes its final report here. This is the audit trail.

**Decisions** (`docs/decisions/`) — the **architect agent must automatically write and commit an ADR immediately after any non-trivial design decision is made**, without waiting to be asked. Use the stencil at `docs/stencils/adr.md`. Number sequentially (0001, 0002, …). Status values: `PROPOSED` → `ACCEPTED` → `SUPERSEDED by #NNNN`.
...
Member: [member name]
Agent: [architect | flutter-engineer | qa-engineer | security-reviewer | release-engineer]
